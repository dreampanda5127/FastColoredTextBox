...
namespace Tester
{
    public partial class PowerfulSample : Form
    {
        ...
        TextStyle SelectedStyle = new TextStyle(null, Brushes.Yellow, FontStyle.Regular);
        MarkerStyle SameWordsStyle = new MarkerStyle(new SolidBrush(Color.FromArgb(40, Color.Gray)));
        public PowerfulSample()
        {
            // Load_Keywords();
            InitializeComponent();
            // fctb.Handle_Bold_By_Keyword(keywords);
        }
        ...
        // MOUSE EVENT HANDLER TO HIGHT SELECTED SENTENCE
        private bool IsMultiLineSelection(FastColoredTextBoxNS.FastColoredTextBox textBox)
        {
            int startLine = textBox.Selection.Start.iLine;
            int endLine = textBox.Selection.End.iLine;

            return startLine != endLine;
        }

        private void find_equal_selected_text(object sender, System.EventArgs e)
        {
            string text = fctb.SelectedText;
            fctb.ClearStyle(StyleIndex.All);
            fctb.OnTextChanged();
            // Load_Keywords();
            // fctb.Handle_Bold_By_Keyword(keywords);
            if (text.Length == 0 || IsMultiLineSelection(fctb))
            {
            }
            else
            {
